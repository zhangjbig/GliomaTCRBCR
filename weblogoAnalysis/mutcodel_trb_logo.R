...
mutcodel <- data.frame(len = 0,prop = 0,CDR3.aa = 0,CGGA_ID = 0)
mutcodel <- mutcodel[-1,]
...
mutcodel <- mutcodel[which(mutcodel$len == 15),]
mutcodel_logo <- data.frame(table(mutcodel$CDR3.aa))
write.csv(mutcodel_logo,"/Users/wanglu/Documents/2021_TC/final_version2/length_weblogo/mutcodel_trb_logo.csv",row.names = FALSE ) 
