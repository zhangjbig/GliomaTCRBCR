...
wt <- data.frame(len = 0,prop = 0,CDR3.aa = 0,CGGA_ID = 0)
wt <- wt[-1,]
...
wt <- wt[which(wt$len == 16),]
wt_logo <- data.frame(table(wt$CDR3.aa))
write.csv(wt_logo,"/Users/wanglu/Documents/2021_TC/final_version2/length_weblogo/wt_igh_logo.csv",row.names = FALSE ) 
