...
mutcodel_Chain <- Chain[,c(3,4,5,6)]
#以下是所有重复的行，只计算一次
...
mutcodel_Chain1 <- unite(mutcodel_Chain,"newcolname","CDR3.aa","V.name","D.name","J.name",sep = ",",remove = FALSE)

public <- mutcodel_Chain1[which(mutcodel_Chain1$newcolname %in% repeat_Chain1$newcolname),]
public <- public[,-1]
public <- unique(public) #17种克隆类型

public$len <- nchar(public$CDR3.aa)
public <- public[,c(1,5)]
public <- unique(public)
public$log2len <- log2(public$len)
write.csv(public,"/Users/wanglu/Documents/2021_TC/final_version2/private_public/private_public_mutcodel/mutcodel_igh_public.csv",row.names = FALSE )

private <- mutcodel_Chain1[-which(mutcodel_Chain1$newcolname %in% repeat_Chain1$newcolname),]
private <- private[,-1] #6088种克隆类型

private$len <- nchar(private$CDR3.aa)
private <- private[,c(1,5)]
private <- unique(private)
private$log2len <- log2(private$len)
write.csv(private,"/Users/wanglu/Documents/2021_TC/final_version2/private_public/private_public_mutcodel/mutcodel_igh_private.csv",row.names = FALSE )
