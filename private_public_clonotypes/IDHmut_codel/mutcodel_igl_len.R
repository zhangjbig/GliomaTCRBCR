...
public <- public[,-1]
public <- unique(public) #113种克隆类型

public$len <- nchar(public$CDR3.aa)
public <- public[,c(1,4)]
public <- unique(public)
public$log2len <- log2(public$len)
write.csv(public,"/Users/wanglu/Documents/2021_TC/final_version2/private_public/private_public_mutcodel/mutcodel_igl_public.csv",row.names = FALSE )

private <- mutcodel_Chain1[-which(mutcodel_Chain1$newcolname %in% repeat_Chain1$newcolname),]
private <- private[,-1] #3419种克隆类型

private$len <- nchar(private$CDR3.aa)
private <- private[,c(1,4)]
private <- unique(private)
private$log2len <- log2(private$len)
write.csv(private,"/Users/wanglu/Documents/2021_TC/final_version2/private_public/private_public_mutcodel/mutcodel_igl_private.csv",row.names = FALSE )
