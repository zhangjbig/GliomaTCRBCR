...
for (i in c(1:nrow(repeat_Chain))){
  for (j in c(1:nrow(wt_Chain))){
    if (sum(repeat_Chain[i,] == wt_Chain[j,]) == ncol(wt_Chain)){
      repeat_line <- rbind(repeat_line,wt_Chain[j,])
      row = rbind(row,j)
    }
  }
}
...
write.csv(public,"/Users/wanglu/Documents/2021_TC/final_version2/private_public/private_public_wt/wt_trb_public.csv",row.names = FALSE )
...
write.csv(private,"/Users/wanglu/Documents/2021_TC/final_version2/private_public/private_public_wt/wt_trb_private.csv",row.names = FALSE )
