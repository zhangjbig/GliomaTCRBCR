...
wt_Chain1 <- unite(wt_Chain,"newcolname","CDR3.aa","V.name","D.name","J.name",sep = ",",remove = FALSE)

public <- wt_Chain1[which(wt_Chain1$newcolname %in% repeat_Chain1$newcolname),]
public <- public[,-1]
public <- unique(public) #350种克隆类型

public$len <- nchar(public$CDR3.aa)
public <- public[,c(1,5)]
public <- unique(public)
public$log2len <- log2(public$len)
write.csv(public,"/Users/wanglu/Documents/2021_TC/final_version2/private_public/private_public_wt/wt_igh_public.csv",row.names = FALSE )

private <- wt_Chain1[-which(wt_Chain1$newcolname %in% repeat_Chain1$newcolname),]
private <- private[,-1] #88579种克隆类型

private$len <- nchar(private$CDR3.aa)
private <- private[,c(1,5)]
private <- unique(private)
private$log2len <- log2(private$len)
write.csv(private,"/Users/wanglu/Documents/2021_TC/final_version2/private_public/private_public_wt/wt_igh_private.csv",row.names = FALSE )
