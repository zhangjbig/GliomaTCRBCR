...
write.csv(public,"/Users/wanglu/Documents/2021_TC/final_version2/private_public/private_public_wt/wt_igl_public.csv",row.names = FALSE )

private <- wt_Chain1[-which(wt_Chain1$newcolname %in% repeat_Chain1$newcolname),]
private <- private[,-1] #41030种克隆类型

private$len <- nchar(private$CDR3.aa)
private <- private[,c(1,4)]
private <- unique(private)
private$log2len <- log2(private$len)
write.csv(private,"/Users/wanglu/Documents/2021_TC/final_version2/private_public/private_public_wt/wt_igl_private.csv",row.names = FALSE )
