...
wt_sim <- unlist(lapply(1:length(names(Chain$data)), FUN = function(x){diversity(Chain$data[[x]]$Clones,"simpson")}))


wt_sim <- as.data.frame(wt_sim)
wt_sim$CGGA_ID <- names(Chain$data)
wt_sim$log2 <- log2(wt_sim$wt_sim)
write.csv(wt_sim,"/Users/wanglu/Documents/2021_TC/final_version2/evaluating_indicator/tra_wt_sim.csv",row.names = FALSE ) 
...
mutcodel_sim <- unlist(lapply(1:length(names(Chain$data)), FUN = function(x){diversity(Chain$data[[x]]$Clones,"simpson")}))
mutcodel_sim <- as.data.frame(mutcodel_sim)
mutcodel_sim$CGGA_ID <- names(Chain$data)
mutcodel_sim$log2 <- log2(mutcodel_sim$mutcodel_sim)
write.csv(mutcodel_sim,"/Users/wanglu/Documents/2021_TC/final_version2/evaluating_indicator/tra_mutcodel_sim.csv",row.names = FALSE ) 
...
mutnoncodel_sim <- unlist(lapply(1:length(names(Chain$data)), FUN = function(x){diversity(Chain$data[[x]]$Clones,"simpson")}))
mutnoncodel_sim <- as.data.frame(mutnoncodel_sim)
mutnoncodel_sim$CGGA_ID <- names(Chain$data)
mutnoncodel_sim$log2 <- log2(mutnoncodel_sim$mutnoncodel_sim)
write.csv(mutnoncodel_sim,"/Users/wanglu/Documents/2021_TC/final_version2/evaluating_indicator/tra_mutnoncodel_sim.csv",row.names = FALSE )
